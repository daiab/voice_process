...
def loadSVModel(SVMmodelName, isRegression=False):
    ...
    with open(SVMmodelName, 'rb') as fid:
        SVM = cPickle.load(fid, encoding='ISO-8859-1')
...
def fileClassification(inputFile, modelName, modelType):
    ...
    [Fs, x] = audio_basic_io.read_audio_file(inputFile)        # read audio file and convert to mono
    ...
    # feature extraction:
    [MidTermFeatures, s] = aF.mt_feature_extraction(x, Fs, mtWin * Fs, mtStep * Fs, round(Fs * stWin), round(Fs * stStep))
